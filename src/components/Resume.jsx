      ...
      <section className="mb-2">
        ...
        <div>
          ...
          <ul className="space-y-0.5">
            ...
            <BulletItem>
              设计<strong>三代语义分析引擎</strong>（<GradientText gradient="gradient-embedding">Embedding</GradientText> 聚类 → <GradientText gradient="gradient-ai">LLM</GradientText> 维度解析 → 结构化提取），处理 13,548 条工程造价清单去重
            </BulletItem>
            <BulletItem>
              逆向分析外部系统 RSA + Shiro 认证，独立编写 <GradientText gradient="gradient-python">Python</GradientText> 脚本实现数据自动化拉取，无需供应商文档
            </BulletItem>
            <BulletItem>
              担任官网技术负责人，主导 <GradientText gradient="gradient-nextjs">Next.js 16</GradientText> + <GradientText gradient="gradient-spring">Spring Boot</GradientText> Monorepo 架构，制定多人协作规范（Q1 绩效满分）
            </BulletItem>
          </ul>
        </div>
      </section>
      ...
      <section className="mb-2">
        ...
        <div className="mb-2">
          ...
          <ul className="space-y-0.5">
            ...
            <BulletItem>
              构建 <strong><GradientText gradient="gradient-ai">LLM</GradientText> 摘要管道</strong>：OpenAI-compatible 无 SDK 直调，支持通义 / Deepseek / Ollama 一键切换；pending 状态追踪 + 错误隔离，单篇失败不阻断整批
            </BulletItem>
            <BulletItem>
              <strong>APScheduler 定时调度</strong>：6 个 Cron 任务 + asyncio 并发锁 + 随机抖动，防采集重叠与突发
            </BulletItem>
            <BulletItem>
              87 个 API 端点，421 个测试用例（覆盖率 72%），16 张数据库表，<GradientText gradient="gradient-docker">Docker</GradientText> Compose 一键部署
            </BulletItem>
          </ul>
        </div>
        ...
        <div className="mb-2">
          ...
          <ul className="space-y-0.5">
            ...
            <BulletItem>
              <strong>Prompt 工程</strong>：双层提示词设计，JSON 平衡括号计数解析（防贪心正则截断），429 限流指数退避重试
            </BulletItem>
            <BulletItem>
              10 并发 LLM + 20 并发 <GradientText gradient="gradient-embedding">Embedding</GradientText> 批处理，60 个预计算缓存任务（<GradientText gradient="gradient-mysql">MySQL</GradientText> 持久化，启动注入 &lt;0.5s）
            </BulletItem>
            <BulletItem>
              后端 17 个服务模块约 9,000 行，32 个 API 端点；前端 <GradientText gradient="gradient-echarts">ECharts</GradientText> 多维可视化（热力图 / 下钻分析）
            </BulletItem>
          </ul>
        </div>
